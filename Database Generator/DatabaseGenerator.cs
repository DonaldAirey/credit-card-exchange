namespace FluidTrade.Core.MiddleTier
{
	...
	class DatabaseGenerator
	{
		...
		private static void GenerateForeignKeys(StreamWriter streamWriter, TableSchema tableSchema)
		{
			...
			// Collect all the foreign key constraints into a handy list.  Emitting the SQL requires a knowledge of the number of
			// foreign key constraints constraints so the DDL can be formatted propertly.
			List<ForeignKeyConstraintSchema> foreignKeyConstraints = new List<ForeignKeyConstraintSchema>();
			foreach (KeyValuePair<string, ConstraintSchema> constraintPair in tableSchema.Constraints)
				if (constraintPair.Value is ForeignKeyConstraintSchema)
					foreignKeyConstraints.Add(constraintPair.Value as ForeignKeyConstraintSchema);

			// This will generate the foreign keys for the table.
			int foreignKeyIndex = 0;
			foreach (ForeignKeyConstraintSchema foreignKeyConstraintSchema in foreignKeyConstraints)
			{

				// The header is emitted with the first foreign constraint.
				...
				// There's no need to reference the parent table if it doesn't reside in the persistent database.
				TableSchema parentTable = foreignKeyConstraintSchema.RelatedTable;
				if (!parentTable.IsPersistent)
					continue;

				streamWriter.WriteLine("		constraint \"{0}\" foreign key ", foreignKeyConstraintSchema.Name);
				streamWriter.WriteLine("		(");

				// Generate the child field names.
				ColumnSchema[] childFields = foreignKeyConstraintSchema.Columns;
				...
				streamWriter.WriteLine("		) references \"{0}\" (", foreignKeyConstraintSchema.RelatedTable.Name);
				ColumnSchema[] parentFields = foreignKeyConstraintSchema.RelatedColumns;
				for (int columnIndex = 0; columnIndex < childFields.Length; columnIndex++)
				{
				...
				// This concludes each statement and provides a seperator for the next.
				streamWriter.WriteLine(foreignKeyIndex++ < foreignKeyConstraints.Count - 1 ? "		)," : "		)");

				// Space each constraint from the next.
				streamWriter.WriteLine();
