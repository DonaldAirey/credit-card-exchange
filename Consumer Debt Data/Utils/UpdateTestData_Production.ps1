#
# DebtTrak tailored scripts for loading the 'Production' DB on a dev system.  This script is made to be run from the 
...
#       directory/Staging area. Once we have a dedicated DebtTrak/production branch, this file will probably end up 
...
set-itemproperty -path "..\Unit Test\Production\*.*" -name attributes -value ([System.IO.FileAttributes]::Normal)
...
copy-item "${PROJDIR}\DebtTrak\Guardian\Database\Unit Test\Production\*.*" "..\Unit Test\Production" -force -verbose
...
& '.\Load Production Data Model.ps1'  

# Bootstrap the DB with generated data 
& '.\Load Production Data.ps1'  
