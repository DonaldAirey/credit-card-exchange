namespace FluidTrade.Guardian
{
    ...
    internal class DebtCross : IExchange
    {
        ...
        private static Thread crossingThread;
        ...
        static DebtCross()
        {
            ...
            DebtCross.syncRoot = new Object();

            // This is the time a user is given to respond to the notification of a possible match.
            DebtCross.negotiationTime = TimeSpan.FromMilliseconds(DebtCross.crossingTime);

            // This queue is filled up with Working Orders that need to be serviced because something changed the matching criteria.
            DebtCross.actionQueue = new WaitQueue<ObjectAction>();

            // This timer is used to service all the time sensitive records used for negotiations and matching.
            DebtCross.timer = new Timer(TimerCallback, null, 0, DebtCross.timerPeriod);
            ...
            using (TransactionScope transactionScope = new TransactionScope())
            {
                ...
                if (destinationRow != null)
                {
                    ...
                    try
                    {
                        ...
                        DebtCross.destinationId = destinationRow.DestinationId;
...

            {
                ...
                while (true)
                {
                    ...
                    try
                    {
                        ...
                        ObjectAction objectAction = DebtCross.actionQueue.Dequeue();
            ...
            using (TransactionScope transactionScope = new TransactionScope())
            {
                ...
                try
                {
                    ...
                    if (workingOrderRow.StatusId == StatusMap.FromCode(Status.Submitted))
                    {
                        ...
                        // There is no need to try to match this security if it isn't a Debt asset.
                        if (securityRow.GetDebtRowsByFK_Security_Debt_DebtId().Length == 0)
                            return;
                        ...
                        Guid partyTypeId = DebtCross.GetPartyTypeId(dataModelTransaction, blotterRow);
                        ...
                        foreach (WorkingOrderRow contraOrderRow in securityRow.GetWorkingOrderRowsByFK_Security_WorkingOrder_SecurityId())
                        {
                            ...
                            Guid counterPartyTypeId = DebtCross.GetPartyTypeId(dataModelTransaction, contraBlotterRow);
                    ...
                    else
                    {
                        ...
                        foreach (MatchRow matchRow in workingOrderRow.GetMatchRows())
                        {
                            ...
                            if (matchRow.StatusId == StatusMap.FromCode(Status.Active))
                            {
                                ...
                                DebtCross.actionQueue.Enqueue(new ObjectAction(DebtCross.CrossWorkingOrder, new Object[] { matchRow.ContraOrderId }));
                ...
                if (isMatched)
                {
                    ...
                    if (matchRow == null)
                    {
                        ...
                        dataModel.CreateMatchTimer(currentTime, matchId, DateTime.Now + DebtCross.negotiationTime);
                    ...
                    else
                    {
                        ...
                        dataModel.CreateMatchTimer(currentTime, matchRow.MatchId, DateTime.Now + DebtCross.negotiationTime);
                    ...
                    if (contraMatchRow == null)
                    {
                        ...
                        dataModel.CreateMatchTimer(currentTime, contraMatchId, DateTime.Now + DebtCross.negotiationTime);
                    ...
                    else
                    {
                        ...
                        dataModel.CreateMatchTimer(currentTime, contraMatchRow.MatchId, DateTime.Now + DebtCross.negotiationTime);
                ...
                foreach (NegotiationRow contraNegotiationRow in contraMatchRow.GetNegotiationRows())
                {
                    ...
                    if (negotiationRow.StatusId == StatusMap.FromCode(Status.Declined) || isWorkingOrderTired)
                    {
                        ...
                        if (traderRow != null)
                        {
                            ...
                            DateTime wakeupTime = currentTime + (traderRow == null ? TimeSpan.FromSeconds(DebtCross.marketSleep) :
        ...
        private static void OnNegotiationRowChanging(object sender, NegotiationRowChangeEventArgs e)
        {
            ...
            if (e.Action == DataRowAction.Commit)
                if (!e.Row.HasVersion(DataRowVersion.Original) && e.Row.RowState != DataRowState.Detached)
                {
                    ...
                    DebtCross.actionQueue.Enqueue(new ObjectAction(DebtCross.NegotiateCross, new Object[] { negotiationId }));
                }
            ...
            if (workingOrderRowChangeEventArgs.Action == DataRowAction.Commit && workingOrderRowChangeEventArgs.Row.RowState != DataRowState.Deleted)
            {
                ...
                if (!workingOrderRow.HasVersion(DataRowVersion.Original))
                {
                    ...
                    if (currentStatus == StatusMap.FromCode(Status.Submitted))
                        DebtCross.actionQueue.Enqueue(new ObjectAction(DebtCross.CrossWorkingOrder, new Object[] { workingOrderId }));
                }
                else
                {
        ...
        public void Start()
        {
            ...
            DebtCross.crossingThread = new Thread(new ThreadStart(DebtCross.CrossingThread));
            DebtCross.crossingThread.Name = "Crossing Thread";
            DebtCross.crossingThread.IsBackground = true;
            DebtCross.crossingThread.Start();
        ...
        public void Stop()
        {
            ...
            if (!DebtCross.crossingThread.Join(DebtCross.threadWait))
                DebtCross.crossingThread.Abort();

        }

        /// <summary>
        /// Handles the time based operations on crossing records.
        /// </summary>
        /// <param name="startData">The generic thread initialization parameter.</param>
        private static void TimerCallback(object startData)
        {
            ...
            //Thread.CurrentPrincipal = DebtCross.claimsPrincipal;

            // An instance of the data model is required for all CRUD operations.
            DataModel dataModel = new DataModel();

            // All timers are updated with the current time.  This synchronizes the activities between the contra parties because they're all working off the
            // same clock.
            DateTime currentTime = DateTime.Now;
            ...
            try
            {
                ...
                // The main idea here is to update each timer with the current time.  When a timer has passed the termination time, the negotiation associated with
                // that timer is implicitly rejected.
                using (TransactionScope transactionScope = new TransactionScope())
                {

                    // This provides a context for all the middle tier transactions.
                    DataModelTransaction dataModelTransaction = DataModelTransaction.Current;

                    // The timer row will be removed from the table when the time expires.  This copy of the table is created so the table's enumeration won't be 
                    // corrpted when elements are removed from the timer table.
                    List<MatchTimerRow> matchTimerRows = new List<MatchTimerRow>();
                    foreach (MatchTimerRow matchTimerRow in DataModel.MatchTimer)
                        matchTimerRows.Add(matchTimerRow);

                    // Once the list of timers is collected from the data model, they are updated with the current server time.  Any timers that have expired will
                    // trigger a record that declines the chance to match with the current contra and that timer row is destroyed.
                    foreach (MatchTimerRow matchTimerRow in matchTimerRows)
                    {

                        try
                        {

                            // The MatchRow record must be locked before it can be examined.
                            matchTimerRow.AcquireReaderLock(dataModelTransaction);

                            // If the current timer has expired then a record is created which declines the opportunity to transact and exchange of a security with
                            // the counter party.  Otherwise, the timer continues to count down.
                            if (currentTime >= matchTimerRow.StopTime)
                            {

                                // This records the fact that the owner of the timer declined the opportunity to trade with the other party in the match.
                                MatchRow matchRow = DataModel.Match.MatchKey.Find(matchTimerRow.MatchId);
                                if (matchRow != null)
                                {

                                    // The Match record must be locked before it can be accessed.
                                    matchRow.AcquireReaderLock(dataModelTransaction);

                                    // If the match is still pending when the timer expires, the potential transaction is implicitly declined.
                                    if (matchRow.StatusId == StatusMap.FromCode(Status.Active))
                                    {

                                        // A record is created that indicates the transaction was implicitly rejected.
                                    ...
                                    // The timer for this potential match is no longer required.
                                    dataModel.DestroyMatchTimer(new Object[] { matchTimerRow.MatchId }, matchTimerRow.RowVersion);
                            ...
                            else
                            {
                                ...
                                // Updating this timer will force it to update the client's data model where the timer can be displayed in the negotiation console.
                                dataModel.UpdateMatchTimer(currentTime, null, new Object[] { matchTimerRow.MatchId }, matchTimerRow.RowVersion, null);
                        ...
                        catch (Exception exception)
                        {
                            ...
                            // Record any generic errors to the event log.
                            EventLog.Error("{0}: {1}", exception.Message, exception.StackTrace);
                    ...
                    // The working orders have a timer that is used to put them to sleep after a negotiation has been declined.  The code below will update each 
                    // of the timers and purge the table of expired timers.  Since the purging will corrupt any enumerator associated with the table, a temporary 
                    // list is made here of all the timers used to put the working orders to sleep.
                    List<WorkingOrderTimerRow> workingOrderTimerRows = new List<WorkingOrderTimerRow>();
                    foreach (WorkingOrderTimerRow workingOrderTimerRow in DataModel.WorkingOrderTimer)
                        workingOrderTimerRows.Add(workingOrderTimerRow);

                    // Once the list of timers is collected from the data model, they are updated with the current server time.  Any timers that have expired will 
                    // trigger a record that wakes up the working order and allows it to look for counter parties.
                    foreach (WorkingOrderTimerRow workingOrderTimerRow in workingOrderTimerRows)
                    {

                        try
                        {

                            // The working order timer must be locked to read it.
                            workingOrderTimerRow.AcquireReaderLock(dataModelTransaction);

                            // When the timer expires, wake up the working order and allow it to look for counter parties and then destroy the timer associated 
                            // with this working order.
                            if (currentTime >= workingOrderTimerRow.StopTime)
                            {
                                WorkingOrderRow workingOrderRow = DataModel.WorkingOrder.WorkingOrderKey.Find(workingOrderTimerRow.WorkingOrderId);
                                workingOrderRow.AcquireReaderLock(dataModelTransaction);
                                dataModel.UpdateWorkingOrder(
                                    null,
                                    null,
                                    ...
                                    null,
                                    new Object[] { workingOrderRow.WorkingOrderId });
                                dataModel.DestroyWorkingOrderTimer(workingOrderTimerRow.RowVersion, new Object[] { workingOrderTimerRow.WorkingOrderTimerId });
                            ...
                            else
                            {
                                ...
                                // Updating this timer will force it to update the client's data model where the timer can be displayed in the negotiation console.
                                dataModel.UpdateWorkingOrderTimer(currentTime, workingOrderTimerRow.RowVersion, null, null, null,
                                    new Object[] { workingOrderTimerRow.WorkingOrderTimerId });
                        ...
                        catch (Exception exception)
                        {
                            ...
                            // Write the generic errors to the event log.
                            EventLog.Error("{0}: {1}", exception.Message, exception.StackTrace);
                    ...
                    // At this point the changes made by the timer can be committed.
                    transactionScope.Complete();
