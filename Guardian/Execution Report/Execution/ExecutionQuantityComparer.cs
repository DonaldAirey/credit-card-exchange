namespace FluidTrade.Guardian.Schema.Execution
{
    ...
    /// </summary>
	public class ExecutionQuantityComparer : System.Collections.Generic.IComparer<Execution>
    {
		...
		public int Compare(Execution operand1, Execution operand2)
        {
			...
			if (operand1.NullSafe(p => p.ExecutionQuantity).NullSafe(p => p.Text) is Decimal)
				operand1Balance = (Decimal)operand1.ExecutionQuantity.Text;

			if (operand2.NullSafe(p => p.ExecutionQuantity).NullSafe(p => p.Text) is Decimal)
				operand2Balance = (Decimal)operand2.ExecutionQuantity.Text;
