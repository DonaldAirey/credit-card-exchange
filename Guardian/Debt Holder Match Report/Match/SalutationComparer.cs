namespace FluidTrade.Guardian.Schema.DebtHolderMatch
{
    ...
    /// </summary>
    public class SalutationComparer : IComparer<Match>
    {
        ...
        public int Compare(Match operand1, Match operand2)
		{
			...
			if (operand1.NullSafe(p => p.Salutation).NullSafe(p => p.Name) is String)
				operand1Salutation = (String)operand1.Salutation.Name;

			if (operand2.NullSafe(p => p.Salutation).NullSafe(p => p.Name) is String)
