namespace FluidTrade.Guardian.Schema.DebtHolderMatch
{
	...
	public class CreditCardBalanceComparer : System.Collections.Generic.IComparer<Match>
	{
		...
		public int Compare(Match operand1, Match operand2)
		{
			...
			if (operand1.NullSafe(p => p.AccountBalance).NullSafe(p => p.Balance) is Decimal)
				operand1Balance = (Decimal)operand1.AccountBalance.Balance;

			if (operand2.NullSafe(p => p.AccountBalance).NullSafe(p => p.Balance) is Decimal)
				operand2Balance = (Decimal)operand2.AccountBalance.Balance;
