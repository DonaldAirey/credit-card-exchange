namespace FluidTrade.Guardian.Schema.CreditCard
{
	...
	/// </summary>
	public class SocialSecurityNumberComparer : IComparer<CreditCard>
	{
		...
		public int Compare(CreditCard operand1, CreditCard operand2)
		{
			...
			if (operand1.NullSafe(p => p.SocialSecurity).NullSafe(p => p.Text) is String)
				operand1SocialSecurityNumber = (String)operand1.SocialSecurity.Text;

			if (operand2.NullSafe(p => p.SocialSecurity).NullSafe(p => p.Text) is String)
				operand2SocialSecurityNumber = (String)operand2.SocialSecurity.Text;
