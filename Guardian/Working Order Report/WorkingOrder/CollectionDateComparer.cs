namespace FluidTrade.Guardian.Schema.ConsumerTrustWorkingOrder
{
	...
	/// </summary>
	public class CollectionDateComparer : IComparer<WorkingOrder>
	{
		...
		{
			if (operand1.CollectionDate.Date == DBNull.Value && operand2.CollectionDate.Date == DBNull.Value)
				return 0;
			if (operand1.CollectionDate.Date == DBNull.Value && operand2.CollectionDate.Date != DBNull.Value)
				return -1;
			if (operand1.CollectionDate.Date != DBNull.Value && operand2.CollectionDate.Date == DBNull.Value)
				return 1;
			return ((DateTime)operand1.CollectionDate.Date).CompareTo((DateTime)operand2.CollectionDate.Date);
		}
