namespace FluidTrade.Guardian.Schema.ConsumerTrustWorkingOrder
{
	...
	/// </summary>
	public class LastPaidDateComparer : IComparer<WorkingOrder>
	{
		...
		{
			if (operand1.LastPaidDate.Date == DBNull.Value && operand2.LastPaidDate.Date == DBNull.Value)
				return 0;
			if (operand1.LastPaidDate.Date == DBNull.Value && operand2.LastPaidDate.Date != DBNull.Value)
				return -1;
			if (operand1.LastPaidDate.Date != DBNull.Value && operand2.LastPaidDate.Date == DBNull.Value)
				return 1;
			return ((DateTime)operand1.LastPaidDate.Date).CompareTo((DateTime)operand2.LastPaidDate.Date);
		}
