namespace FluidTrade.Guardian.Schema.DebtNegotiatorWorkingOrder
{
	...
	/// </summary>
	public class MiddleNameComparer : IComparer<WorkingOrder>
	{
		...
		public int Compare(WorkingOrder operand1, WorkingOrder operand2)
		{
            ...
            if (operand1.NullSafe(p => p.MiddleName).NullSafe(MiddleName => MiddleName.Name) is String)
                operand1MiddleName = (String)operand1.MiddleName.Name;

            if (operand2.NullSafe(p => p.MiddleName).NullSafe(MiddleName => MiddleName.Name) is String)
