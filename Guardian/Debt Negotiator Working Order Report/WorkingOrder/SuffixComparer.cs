namespace FluidTrade.Guardian.Schema.DebtNegotiatorWorkingOrder
{
	...
	/// </summary>
	public class SuffixComparer : IComparer<WorkingOrder>
	{
		...
		public int Compare(WorkingOrder operand1, WorkingOrder operand2)
		{
            ...
            if (operand1.NullSafe(p => p.Suffix).NullSafe(Suffix => Suffix.Name) is String)
                operand1Suffix = (String)operand1.Suffix.Name;

            if (operand2.NullSafe(p => p.Suffix).NullSafe(Suffix => Suffix.Name) is String)
