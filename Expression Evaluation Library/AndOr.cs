namespace FluidTrade.ExpressionEvaluation
{
	...
	internal class AndOrElement : BinaryExpressionElement
	{
		...
		private static void EmitLogicalShortCircuit(FleeILGenerator ilg, ShortCircuitInfo info, IServiceProvider services)
		{
			...
			{
				// Get the operator
				AndOrElement op = info.Operators.Pop() as AndOrElement;
				// Get the left operand
				ExpressionElement leftOperand = info.Operands.Pop() as ExpressionElement;
		...
		private static Label GetShortCircuitLabel(AndOrElement current, ShortCircuitInfo info, FleeILGenerator ilg)
		{
			...
			Stack cloneOperands = info.Operands.Clone() as Stack;
			Stack cloneOperators = info.Operators.Clone() as Stack;
			...
			while (cloneOperators.Count > 0)
			{
				...
				AndOrElement top = cloneOperators.Pop() as AndOrElement;
